...
public class Service {
    ...
    public Component sortWordInSentenceByLength(Component text) {
        ...
        for (Component sentence : component.getComponents()
        ) {
            for (Component lexeme : ((Composite) sentence).getComponents()
            ) {
                ((Composite) lexeme).getComponents().sort(Comparator.comparing(component1 -> component1.operation().length()));
            }
        }
        return component;
    }

    public int count(Component component, Type type) {
        int count = 0;
